...
def speed_camera():
    ...
    #Connecting obd
    ser = serial.Serial('/dev/tty3', 38400, timeout=1)
    connection=obd.OBD(baudrate=38400,fast=False)
    
    #checking obd connection
    if connection.is_connected():
        print("OBD Connected Successfully!")
        
        cmd=obd.commands.RPM
        resRPM=connection.query(cmd)
        #Engine temprature warning
        cmd=obd.commands[1][5]
        resEngineTemprature=connection.query(cmd)
        
        if( getInteger(resEngineTemprature) >=105):
            os.system("omxplayer /home/pi/speed-camera/instructions/EngineTempratureWarning.mp3")
        
    else:
        vs.stop()
        os.system("omxplayer /home/pi/speed-camera/instructions/OBDNotConnectedWarning.mp3")
        sys.exit(1)
            ...
            if motion_found:
                ...
                if first_event:   # This is a first valid motion event
                    ...
                    if (abs(cx - end_pos_x) > x_diff_min and
                        ...
                        file = open("speed-camera/profile.txt", 'a')
                        file.write("Speed of next object is\t:\t")
                        file.write(str(ave_speed))
                        file.write(" ")
                        file.write(speed_units)
                        #getting current vehicle speed
                        cmd=obd.commands.SPEED
                        resSpeed=connection.query(cmd)
                        file.write("current Vehicle speed is : ")
                        file.write(str(resSpeed))
                        file.write("\n")
                        file.close()
                        if(ave_speed < 1.0):
                            os.system("omxplayer /home/pi/speed-camera/instructions/CollisionWarning.mp3")
